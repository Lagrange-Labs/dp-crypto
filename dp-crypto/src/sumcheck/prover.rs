...
use crate::{
    ...
    extrapolate::{ExtrapolationCache, extrapolate_from_table},
    monomial::Term,
    structs::{IOPProof, IOPProverMessage, IOPProverState},
    util::{
        AdditiveArray, AdditiveVec, ceil_log2, largest_even_below, merge_sumcheck_polys,
...
struct Phase1Workers<'a, F: PrimeField> {
    workers_states: Vec<Phase1WorkerState<'a, F>>,
}

impl<'a, F: PrimeField> Phase1Workers<'a, F> {
    fn run(
        mut self,
        num_variables: usize,
        max_degree: usize,
        transcript: &mut impl Transcript,
    ) -> (Vec<IOPProverState<'a, F>>, Vec<IOPProverMessage<F>>) {
        ...
        for _ in 0..num_variables {
            ...
            transcript.append_scalars(&evaluations.0);
...
struct Phase1WorkerState<'a, F: PrimeField> {
    prover_state: IOPProverState<'a, F>,
    challenge: Option<F>,
}

impl<'a, F: PrimeField> Phase1WorkerState<'a, F> {
    fn new(
        is_main_worker: bool,
...
impl<'a, F: PrimeField> IOPProverState<'a, F> {
    ...
    pub fn prove(
        virtual_poly: VirtualPolynomials<'a, F>,
        transcript: &mut impl Transcript,
    ) -> (IOPProof<F>, IOPProverState<'a, F>) {
        ...
        if min_degree < max_degree {
            ...
            let _ = ExtrapolationCache::<F>::get(min_degree, max_degree);
        }

        transcript.append_bytes(&(num_variables + log2_max_thread_id).to_le_bytes());
        transcript.append_bytes(&max_degree.to_le_bytes());
        ...
        for _ in 0..log2_max_thread_id {
            ...
            challenge = Some(transcript.append_and_sample(b"Internal round"));
        }
        exit_span!(span);
    ...
    fn phase1_sumcheck(
        ...
        mut polys: Vec<VirtualPolynomial<'a, F>>,
        max_degree: usize,
        transcript: &mut impl Transcript,
    ) -> (Vec<IOPProverState<'a, F>>, Vec<IOPProverMessage<F>>) {
        let log2_max_thread_id = ceil_log2(max_thread_id); // do not support SIZE not power of 2
    ...
    pub fn prover_init_with_extrapolation_aux(
        is_main_worker: bool,
        polynomial: VirtualPolynomial<'a, F>,
        ...
        let AdditiveVec(uni_polys) = self.poly.products.iter().fold(
            ...
            |mut uni_polys, MonomialTerms { terms }| {
                ...
                } in terms
                {
                    ...
                    let mut uni_variate = vec![F::ZERO; self.poly.aux_info.max_degree + 1];
                    let uni_variate_monomial: Vec<F> = match prod.len() {
                    ...
                    uni_variate
                        ...
                        .take(prod.len() + 1)
                        .for_each(|(eval, monomial_eval)| *eval = monomial_eval * *scalar);
    ...
    pub fn prove_parallel(
        ...
        transcript.append_bytes(&num_variables.to_le_bytes());
        transcript.append_bytes(&max_degree.to_le_bytes());
        ...
        if let Some(p) = challenge {
            prover_state.push_challenges(vec![p]);
            // fix last challenge to collect final evaluation
            prover_state.fix_var_parallel(&p);
        };
        exit_span!(span);
    ...
    /// Initialize the prover state to argue for the sum of the input polynomial
    /// over {0,1}^`num_vars`.
    pub(crate) fn prover_init_parallel(polynomial: VirtualPolynomial<'a, F>) -> Self {
        let start = entered_span!("sum check prover init");
        assert_ne!(
            polynomial.aux_info.max_num_variables, 0,
            "Attempt to prove a constant."
    ...
    pub fn get_mle_final_evaluations(&self) -> Vec<Vec<F>> {
        self.poly
    ...
    pub fn get_mle_flatten_final_evaluations(&self) -> Vec<F> {
    ...
    /// fix_var
    pub fn fix_var_parallel(&mut self, r: &F) {
        let expected_numvars_at_round = self.expected_numvars_at_round();
        self.poly
            .flattened_ml_extensions
            .par_iter_mut()
...
impl<F: PrimeField> IOPProverState<'_, F> {
    pub fn push_challenges(&mut self, challenge: Vec<F>) {
        self.challenges.extend(challenge)
    }

    pub fn collect_raw_challenges(&self) -> Vec<F> {
