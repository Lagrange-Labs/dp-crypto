...
pub struct VirtualPolynomialsBuilder<'a, F: PrimeField> {
    num_witin: WitnessId,
    mle_ptr_registry: BTreeMap<usize, (usize, EitherRefMLE<'a, DensePolynomial<'a, F>>)>,
    num_threads: usize,
    max_num_variables: usize,
    _phantom: PhantomData<F>,
}

impl<'a, F: PrimeField> VirtualPolynomialsBuilder<'a, F> {
    ...
    pub fn new_with_mles(
        num_threads: usize,
        max_num_variables: usize,
        mles: Vec<EitherRefMLE<'a, DensePolynomial<'a, F>>>,
    ) -> Self {
    ...
    pub fn to_virtual_polys_with_monomial_terms(
        self,
        monimial_terms: &[Term<Expression<F>, Expression<F>>],
        public_io_evals: &[F],
        challenges: &[F],
    ) -> VirtualPolynomials<'a, F> {
        let mles = self
            .mle_ptr_registry
            .into_values()
        ...
        let mut virtual_polys =
            VirtualPolynomials::<F>::new(self.num_threads, self.max_num_variables);
...
impl<'a, F: PrimeField> VirtualPolynomials<'a, F> {
    pub fn new(num_threads: usize, max_num_variables: usize) -> Self {
    ...
    fn register_mles(&mut self, mles: Vec<EitherRefMLE<'a, DensePolynomial<'a, F>>>) -> Vec<usize> {
        let log2_num_threads = log2_strict(self.num_threads);
        let mut indexes = vec![];
        for mle in mles {
    ...
    fn add_monomial_terms(&mut self, monomial_terms: MonomialTermsExpr<F>) {
        self.polys
            .iter_mut()
    ...
    pub fn get_batched_polys(self) -> (Vec<VirtualPolynomial<'a, F>>, Vec<PolyMeta>) {
    ...
    pub fn evaluate_slow(&self, point: &[F]) -> anyhow::Result<F> {
        ...
        let raw_mles_evals = (0..self.polys[0].flattened_ml_extensions.len())
            .map(|index| match self.poly_meta[&index] {
                PolyMeta::Normal => {
                    ...
                    let mut evaluations = first.evals();
                    for mle in iter {
                        evaluations.append(mle.evals().as_mut());
                    }
